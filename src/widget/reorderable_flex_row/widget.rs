...
use iced::alignment;
use iced::window;
use iced::{Alignment, Color, Pixels};
...
/// A keyed wrapping flex row with drag-to-reorder behavior.
...
where
    Key: Clone + Eq + Hash + 'static,
{
    /// Creates an empty reorderable row.
    ...
    /// Creates a row with reserved capacity.
    ...
    /// Sets the padding around the row.
    ...
    /// Sets the vertical alignment of item contents.
    pub fn align_y(mut self, align: impl Into<alignment::Vertical>) -> Self {
        self.align = Alignment::from(align.into());
        self
    }

    /// Sets whether overflow should be clipped.
    ///
    /// Leave this disabled if you want the dragged item to visibly lift above the row.
    pub fn clip(mut self, clip: bool) -> Self {
        self.clip = clip;
        self
    }

    /// Sets how far the dragged item lifts above the row.
    pub fn drag_lift(mut self, lift: f32) -> Self {
        self.drag_lift = lift.max(0.0);
        self
    }

    /// Sets the duration used for sibling slot animation.
    ...
    /// Adds an item that stays fixed in the row order.
    ...
    /// Extends the row with multiple keyed items.
    ...
    /// Extends the row with multiple fixed-position keyed items.
...
impl<'a, Key, Message> Widget<Message, Theme, Renderer> for ReorderableFlexRow<'a, Key, Message>
where
    Key: Clone + Eq + Hash + 'static,
    Message: 'a,
{
...
/// Creates an empty reorderable flex row.
...
fn draw_drag_backdrop(renderer: &mut Renderer, theme: &Theme, bounds: Rectangle) {
